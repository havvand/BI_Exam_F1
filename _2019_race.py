...
df_race_2019 = df_race_2019[(df_race_2019['milliseconds'] <= 200000) & (df_race_2019['milliseconds'] >= 66000)]

fig, axs = plt.subplots(1, 1, figsize=(30, 20))
sns.histplot(df_race_2019['milliseconds'], bins=150)
plt.xticks(np.arange(66000, 200000, 1000))

df_race_2019.describe()

Q1 = df_race_2019['milliseconds'].quantile(0.25)
Q3 = df_race_2019['milliseconds'].quantile(0.75)
IQR = Q3 - Q1
print(Q1, Q3, IQR)
...
driver_race_times = df_race_2019.groupby('driverId')[['driverId', 'raceId', 'lap', 'milliseconds']].apply(lambda x: x)
driver_race_times['driverId_cat'] = pd.Categorical(df_race_2019['driverId']).codes
...
'''
track_stats = df_race_2019.groupby('raceId')['milliseconds'].agg(['mean', 'std', 'min', 'max'])
threshold = 3
...
#%% Scatterplot
race_ids = df_race_2019['raceId'].unique()
driver_ids = df_race_2019['driverId'].unique()
...
With scores of ;
Silhouette Score: 0.5224411673141098:  indicating decent separation between clusters and internal cohesion within clusters.
Calinski Harabasz Score: 225822.2829026921: clusters are very dense and well-separated.
Davies Bouldin Score: 0.5324242833015836:  indicating some similarity between clusters. However, given the clear visual separation in the scatter plot, this might not be a major concern.
